...
fn exercise7() {
    let mut v: Vec<&str> = Vec::new();
    {
        ...
        let s: &'static str = Box::leak(Box::new(std::str::from_utf8(&chars).unwrap().to_owned()));
        v.push(s);
    }
    println!("{:?}", v);
}
...
// Make it compile
fn exercise8() {
    let mut accounting:Vec<String> = Vec::new();
    accounting.push("Alice".to_string());
    accounting.push("Ben".to_string());

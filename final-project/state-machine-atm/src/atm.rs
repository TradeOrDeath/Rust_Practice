...
impl StateMachine for Atm {
    ...
    fn next_state(starting_state: &Self::State, t: &Self::Transition) -> Self::State {
        match (starting_state, t) {
            ...
            (Auth::Authenticating(_), Action::PressKey(Key::Enter)) => Auth::Authenticated,
            (Auth::Authenticated, Action::PressKey(Key::Enter)) => Auth::Waiting,
            (state, _) => state.clone(), // If the transition is not allowed, stay in the same state
        }
    }
}
